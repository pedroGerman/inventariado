...
const tabs = [
  { id: "home", href: "/", label: "Inicio", icon: Home, match: (p: string) => p === "/" },
  {
    id: "ordenes",
    href: "/ordenes",
    label: "Órdenes",
    icon: ClipboardList,
    match: (p: string) =>
      p.startsWith("/ordenes") || p.startsWith("/compras/ordenes"),
  },
  {
    id: "carrito",
    href: "/ventas/carrito",
...
export function BottomNav() {
  ...
  const ordenesHref = isPurchaseFlow ? "/ordenes?tab=purchase" : "/ordenes";
  const cajaHref = isPurchaseFlow ? "/ventas/caja?tab=purchase" : "/ventas/caja";
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-slate-200 bg-white safe-bottom">
      <div className="mx-auto flex max-w-mobile items-center justify-around px-2 pb-2 pt-1">
        {tabs.map((tab) => {
          ...
          const Icon = tab.icon;
          let href = tab.href;
          if (tab.badge) href = carritoHref;
          else if (tab.id === "ordenes") href = ordenesHref;
          else if (tab.id === "caja") href = cajaHref;
            ...
            >
              <div className="relative">
                ...
                {tab.badge && itemCount > 0 && (
                  <span className="absolute -right-3 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-danger p-1 text-[9px] font-bold text-white">
